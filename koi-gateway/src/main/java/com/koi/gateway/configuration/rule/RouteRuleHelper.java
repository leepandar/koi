...
import com.koi.gateway.rest.domain.RouteRule;
import com.koi.gateway.route.RedisRouteDynamicGatewayService;
...
public class RouteRuleHelper {
    ...
    public void saveOrUpdate(RouteRule rule) {
        ...
        stringRedisTemplate.opsForHash().put(GatewayRule.Constants.GATEWAY_RULE_ROUTE, rule.getId(), JSON.toJSONString(rule));
    }

    private boolean publish(String id) {
        final List<String> services = discoveryClient.getServices();
        final Object object = stringRedisTemplate.opsForHash().get(GatewayRule.Constants.GATEWAY_RULE_ROUTE, id);
        if (object == null) {
            return false;
        }
    ...
    public List<RouteRule> query() {
        ...
        final List<String> services = discoveryClient.getServices();
        List<RouteRule> routeRules = stringRedisTemplate.opsForHash().keys(GatewayRule.Constants.GATEWAY_RULE_ROUTE).stream()
                .map(id -> {
                    Object object = stringRedisTemplate.opsForHash().get(GatewayRule.Constants.GATEWAY_RULE_ROUTE, id);
                    if (object == null) {
                        return null;
                    }
                    RouteRule rule = JSON.parseObject(object.toString(), RouteRule.class);
    ...
    public void delete(String id) {
        stringRedisTemplate.opsForHash().delete(GatewayRule.Constants.GATEWAY_RULE_ROUTE, id);
        redisRouteDynamicGatewayService.delete(id);
    }
}
